...
- **`release-it`** with **`@release-it/conventional-changelog`** cuts releases, configured in
  `.release-it.json`. It publishes through `pnpm`, not npm, and authenticates by npm Trusted
  Publishing (OIDC) — so the workflow deliberately sets no `NODE_AUTH_TOKEN`, and `npm.skipChecks` is
  on because release-it's checks shell out to `npm whoami`, which has nothing to answer with when
  there is no token. The CHANGELOG intro lives in the plugin's `header` option, because the plugin
  strips and re-prepends only the header it is given and would otherwise bury any prose beneath the
  newest release. pnpm is pinned to 10 in the workflow: 11 404s on OIDC publish (pnpm/pnpm#11513).
...
a `!` — so it is cheap, and it is not a substitute for `check` and `test`.
...
- Root-owned files: [CONTEXT.md](./CONTEXT.md) (glossary), [docs/adr/](./docs/adr/) (decision records),
  ...
  Frontier consumer friction), [README.md](./README.md) (the package's public face — install, pinning,
  and how a release is cut), [CHANGELOG.md](./CHANGELOG.md) (package release notes, generated by
  release-it; not tracker vocabulary).
