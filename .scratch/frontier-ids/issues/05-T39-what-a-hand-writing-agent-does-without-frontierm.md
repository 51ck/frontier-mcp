...
That reading is not an inference. It is recorded as a field observation, in the test that pins this paragraph (`test/ship.test.ts:134-135`):
...
**An agent with the server loaded.** The failure that produced this paragraph was never a serverless agent making a dangerous choice; it was an agent ignoring an available tool. Nothing in this Effort touches that, and no id format could. The preamble survives whole.
