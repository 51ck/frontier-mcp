...
- The Board's warnings almost all carry a remedy. `collectWarnings` (`src/tools/get-board.ts:150`) emits eight classes — dangling Edges, blocked-by-dropped, unrecognized status, coarsened sub-slice refs, declined-but-takeable, stale claims, Legacy Tickets, missing ids — and seven of them name something a reader can act on, several stating the action outright ("Run migrate_effort to normalize", "ask the holder").

  The eighth is worth facing rather than glossing, because it is the closest thing to a precedent for what this Ticket wants. **Declined-but-takeable is a disposition, not a defect** — somebody set `wontfix` deliberately, and `src/frontier.ts:29` is explicit that the Board reports it "leaving the judgement to the reader". So the channel is not purely for anomalies, and the argument cannot rest on saying it is.

  What separates it from a pattern warning is scope and decision. Declined-but-takeable is **per-Ticket** and puts a live choice in front of the reader: this one is takeable, somebody declined it, decide. It varies from Board to Board and from Ticket to Ticket, and it is gone once the Ticket is taken or retriaged. A pattern warning is **repo-wide**, identical on every Board forever, and asks for no decision at all because the decision was already made in `frontier.yml`. That is the line: the Board's warnings vary with the Board, and a pattern warning is the first that would not.
- **A member that never varies would train readers to skip the block.** That is the real cost, and it is not paid by this warning; it is paid by the ones already there. Every reader who learns the warnings block always opens with the same unchanging line learns to skim it, and what gets skimmed is the dangling Edge and — once [[T38]] is built — the duplicate id. Buying loudness for a standing property by spending the credibility of the channel that reports what changed is a bad trade in the only currency that matters here.

**`update_ticket` is the wrong tool** even though it already renders a `warnings:` block from `TicketWriteResult.warnings` (`src/tools/update-ticket.ts:215`). It does not mint. But its existence settles the shape question: a non-fatal advisory riding a write, produced below the storage seam and rendered by the tool, is an established pattern here rather than an invention — and it is the right seam, since [[T36]] left only the driver knowing what an id looks like.

**The mechanics.** `renderCreated(effort, created)` (`src/tools/create-tickets.ts:221`) takes no workspace and no config, so the advisory arrives either as a new parameter or composed at the `src/server.ts:236` call site. Appending after the last Ticket line is shape-safe: the header states the count up front, so a caller parsing *root line, count line, N Ticket lines* is unaffected by anything after line `2 + N`. The text should match `update_ticket`'s write-side format — `warnings:` then `- ` bullets — rather than `get_board`'s two-space indent, because it is a write. That the two formats differ at all is a small existing wart, and this Ticket should not add a third.
