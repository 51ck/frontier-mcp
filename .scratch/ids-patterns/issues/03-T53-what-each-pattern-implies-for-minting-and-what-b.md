...
blocked_by: []
answer_gist: "Charting fused two independent questions: `<N>` decides whether minting needs coordination, and the presence of any random token decides whether identity survives a merge — so guards are kept for every pattern containing `<N>` (they were never actually deleted) while a mixed pattern like `T<N>-<b36{4}>` is cross-tree safe with only its counter racing, provided the guard keys on the `<N>` value rather than the rendered id; T38's write-refusal cannot substitute because the only path that is a pure function of the id is the guard itself, so \"make the write atomic\" reduces to reinventing it"
---
...
**A pattern containing `<N>` mints under guards even when it also contains a random token.** The randomness would carry uniqueness on its own, so the guard is not load-bearing for identity there. It is kept anyway, because a consumer who writes `<N>` into a pattern is asking for a counter, and silently handing out `35` twice is the same class of broken promise as handing out an id twice — merely cheaper to live with.

**The guard must key on the `<N>` value alone, not on the rendered id.** This is the one place the existing mechanism does not survive the decision unchanged, and it is easy to miss. `guardFor(storage, id)` (`src/storage/markdown/create.ts:350`) builds `.frontier-id-<id>.guard` from the **full** id, which is correct today because `claim()` renders `` `T${candidate}` `` (`:306`) and the id *is* the counter. Under `T<N>-<b36{4}>` two sessions both reaching `35` would render `T35-ab12` and `T35-cd34`, derive two *different* guard paths, and never contend — the guard would serialize nothing, and the counter it was kept for would be handed out twice anyway.

So the guard's subject is restated: it protects the **derived component**, not the id. For a pure-`<N>` pattern that is the same object and nothing changes. For a mixed pattern it is the number, and both racers derive `.frontier-id-N-35.guard` from it and contend as intended. This costs one change — what `guardFor` is passed — and without it the third row of the table above is a recommendation the machinery cannot honour.

The rest of the machinery is already built and already paid for, so declining to use it buys nothing.

## The guards stay, because they were never removed

[[T37]] resolved that ADR 0005's allocation machinery is *deleted outright*, but that deletion was never performed. All of it is still in the tree: `guardFor()` at `src/storage/markdown/create.ts:350`, the exclusive `wx` create in `hold()` at `:334`, the bump-on-collision recursion in `claim()` at `:289`, `release()` at `:280`, and `reserve()` at `:248` still doing scan → take every guard → **re-scan while holding them** → restart if any candidate turned up → hold across the whole write and release in a `finally`. `withIdReservations()` at `:102` wraps it, and `migrate_effort` mints through the same path (`src/storage/markdown/migrate.ts:41`).

*Correcting the Question above:* it credits the guards with having "measured four processes creating three Tickets each producing thirteen files carrying four distinct ids". That is inverted. ADR 0005 reports thirteen files carrying four distinct ids as the result of **the same test run against a naive `max + 1`** — the failure the guards prevent, not an outcome they produce. What the guards measure is the opposite: `test/cross-process-create.test.ts` asserts every session returns `OK` and every id is distinct. The inversion matters here because the number was being used to size how bad losing the guards would be, and it is evidence for keeping them rather than against.
...
- `withIdReservations()` already short-circuits on `count === 0` (`create.ts:108`), so a random pattern can route through the existing signature without taking a guard. The conditional is smaller than it sounds.
...
- `peekMintedIds()` (`create.ts:122`) takes no guards because it serves `migrate_effort`'s preview, which is advisory. Under a random pattern a preview cannot predict what the real run will mint at all, and should say so rather than showing candidates it will not honour.
